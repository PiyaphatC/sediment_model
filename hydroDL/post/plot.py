...
def plotBoxFig(data,
   ...
   # lowlim = [-1.75, 0.4, 0.4, 0.8, 0.6, 0.8]
   # highlim = [0.6, 2.75, 2.75, 1.01, 1.01, 1.01]
   # step = [0.25, 0.2, 0.2, 0.025, 0.05, 0.025]
    for k in range(0, nc):
...
def plotMap(data,
    ...
    if isGrid is True:
        ...
        cs = mm.scatter(
            x, y, c=data, s=120, cmap=plt.cm.jet, vmin=vmin, vmax=vmax)
...
def TempSeries_4_Plots_ERL(attr_path, statDictLst_res, obs, predLst, TempTarget, tRange, boxPlotName, rootOut, save_path, sites=18, Stations=None, retrained=False):
    fig, axes = plt.subplots(2, 2, figsize=(13, 29), constrained_layout=True)
    axes = axes.flat
    npred = 2  # 2  # plot the first two prediction: Base LSTM and DI(1)
    #subtitle = ['(seg_id_nat:1450)', '(seg_id_nat:1566)', '(seg_id_nat:1718)', '(seg_id_nat:2013)']
    txt = ['a', 'b', 'c', 'd']
    ...
    for i in range(math.ceil(len(gage) / 4)):
        ...
        print(gageindex)
        t = utils.time.tRange2Array(tRange)
        fig, axes = plt.subplots(1, 1, figsize=(13, 29), constrained_layout=True)
        #axes = axes.flat
        npred = 4  # 2  # plot the first two prediction: Base LSTM and DI(1)
        if i < (math.ceil(len(gage) / 4) - 1):
            subtitle = ['(a) (Station ID:' + str(seg_id_nat[4 * i]) + ') ',
                        '(b) (Station ID:' + str(seg_id_nat[4 * i + 1]) + ') ',
                        '(c) (Station ID:' + str(seg_id_nat[4 * i + 2]) + ') ',
        ...
        for k in range(1):#len(gageindex)):
            iGrid = AA[gageindex[k]]
            iGrid = inputdata.index[inputdata['site_no'] == seg_id_nat[AA[gageindex[k]]]].values[0]
            yPlot = [obs[iGrid, :]]
            for y in predLst[0:npred]:
            ...
            subtitle1 = '[' + str(np.round(statDictLst_res[0]['RMSE'][iGrid], 2)) + ',' + str(
                np.round(statDictLst_res[0]['NSE'][iGrid], 2)) + ',' + str(
                np.round(statDictLst_res[0]['Bias'][iGrid], 2)) + ',' + str(
                np.round(statDictLst_res[0]['NSE_res'][iGrid], 2)) + ','+ str(
                np.round(statDictLst_res[0]['Corr_res'][iGrid], 2)) + ']'
             #NSE_LSTM = [] #str(round(statDictLst[0]['NSE'][iGrid], 2))
            # NSE_DI1 = str(round(statDictLst[1]['NSE'][iGrid], 2))
            ...
            plotTS(
                ...
                cLst='bkrmg',
                markerLst='o---+1',
                legLst=['obs', 'LSTM_obsQ', 'LSTM_simQ', 'LSTM_noQ'], title=subtitle[k]+subtitle1, linespec=['o', '-', '-', '-', ':', '+'],  #legLst=[TempTarget, 'LSTM: ' + NSE_LSTM], title=subtitle[k]
                ylabel=ylabel)  # ['USGS', 'LSTM: '+NSE_LSTM, 'DI(1): '+NSE_DI1]
...
def plotMap_separate(data, ind99_dam, ind99_nodam,
    ...
    if isGrid is True:
        ...
        cs = mm.scatter(x, y, data, cmap=plt.cm.jet, vmin=vmin, vmax=vmax)
        cs10_60_dam = mm.scatter(
            x10_60_dam, y10_60_dam, c=data10_60_dam, s=40, marker='o', cmap=plt.cm.jet, vmin=vmin, vmax=vmax)
        cs10_60_nodam = mm.scatter(
            x10_60_nodam, y10_60_nodam, c=data10_60_nodam, s=40, marker='s', cmap=plt.cm.jet, vmin=vmin, vmax=vmax)
        cs99_dam = mm.scatter(
            x99_dam, y99_dam, c=data99_dam, s=160, marker='o', cmap=plt.cm.jet, vmin=vmin, vmax=vmax)
        cs99_nodam = mm.scatter(
            x99_nodam, y99_nodam, c=data99_nodam, s=160, marker='s', cmap=plt.cm.jet, vmin=vmin, vmax=vmax)
        cs60_99_dam = mm.scatter(
            x60_99_dam, y60_99_dam, c=data60_99_dam, s=100, marker='o', cmap=plt.cm.jet, vmin=vmin, vmax=vmax)
        cs60_99_nodam = mm.scatter(
            x60_99_nodam, y60_99_nodam, c=data60_99_nodam, s=100, marker='s', cmap=plt.cm.jet, vmin=vmin, vmax=vmax)
    if shape is not None:
        crd = np.array(shape.points)
        par = shape.parts
        if len(par) > 1:
    ...
    if pts is not None:
        ...
        for k in range(npt):
            plt.text(
                ...
                fontsize=18)
    if clbar is True:
        cbar = mm.colorbar(cs10_60_dam, pad='1%')
        cbar.ax.tick_params(labelsize=20)
    if title is not None:
        ax.set_title(title, fontsize=26)
    ...
    legend2 = ax.legend(handles=[cs99_dam, cs99_nodam, cs60_99_dam, cs60_99_nodam, cs10_60_dam, cs10_60_nodam],
              labels=['(p>99)', '(p>99)', '(60<p<99)', '(60<p<99)', '(10<p<60)', '(10<p<60)'],
              loc='lower right',
              fontsize=18)
    if ax is None:
        return fig, ax, mm
